...
- **示例锚点**：示例需编号，格式为 `{#exr-章节号_小节号_序号}`。
- **示例答案锚点**：习题需编号，格式为 `{#sol-章节号_小节号_序号}`。
- **引用语法**：使用 `@sec-ID`、`@eq-ID`、`@thm-ID`、`@def-ID`、`@cor-ID`、`@exr-ID`（示例）、`@sol-ID`（解答）、`@tbl-ID`（表格）、`@fig-ID`（图形）等进行自动交叉引用。**禁止使用纯文本“示例 X.X”进行引用**。
